...
public class DataSeeder implements CommandLineRunner {
    ...
    private void seedCatalogData() {
        // ===== THỂ LOẠI =====
        TheLoai vanHoc = TheLoai.builder().tenTheLoai("Văn học").moTa("Sách văn học trong và ngoài nước").build();
        TheLoai khoaHoc = TheLoai.builder().tenTheLoai("Khoa học").moTa("Sách khoa học tự nhiên và xã hội").build();
        TheLoai lichSu = TheLoai.builder().tenTheLoai("Lịch sử").moTa("Sách lịch sử Việt Nam và thế giới").build();
        TheLoai kyThuat = TheLoai.builder().tenTheLoai("Kỹ thuật - Công nghệ").moTa("Sách lập trình, kỹ thuật, CNTT")
                .build();
        TheLoai thieuNhi = TheLoai.builder().tenTheLoai("Thiếu nhi").moTa("Sách dành cho thiếu nhi").build();
        TheLoai kinhTe = TheLoai.builder().tenTheLoai("Kinh tế").moTa("Sách kinh tế, tài chính, quản trị").build();
        ...
        TacGia toHoai = TacGia.builder().hoDem("Tô").ten("Hoài").tieuSu("Nhà văn Việt Nam, tác giả Dế Mèn Phiêu Lưu Ký")
                .build();
        ...
        TacGia namQuocChanh = TacGia.builder().hoDem("Nam Quốc").ten("Chánh").tieuSu("Tác giả sách kỹ năng và kinh tế")
                .build();
        TacGia danBrown = TacGia.builder().hoDem("Dan").ten("Brown")
                .tieuSu("Nhà văn người Mỹ nổi tiếng với các tiểu thuyết bí ẩn").build();
        tacGiaRepository.saveAll(Arrays.asList(nguyenNhatAnh, toHoai, nguyenDu, fujikoFFujio, namQuocChanh, danBrown));
            ...
            if (!cuonSachSanSang.isEmpty()) {
                ...
                if (!cuonSachSanSang2.isEmpty()) {
                    ...
                    ChiTietPhieuMuon chiTietQuaHan = ChiTietPhieuMuon.builder()
                            ...
                            .cuonSach(cuonQuaHan)
                            .tinhTrangLucMuon(TinhTrangVatLy.BINH_THUONG)
                            .donGiaPhatApDung(cuonQuaHan.getSach().getDonGiaPhatTheoNgay() != null
                                    ? cuonQuaHan.getSach().getDonGiaPhatTheoNgay()
                                    : 5000.0)
